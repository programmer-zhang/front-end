...
* 如果是核心模块，就直接返回模块
* 如果是带有路径的地址，则拼接出一个绝对路径，然后先读取缓存require.cache,再读取文件，如果没有后缀，则自动加后缀后一一识别
	* .js 解析为JavaScript文本文件(上面我们讲了为什么会先解析成文本文件)
	* .json 解析为JSON对象
	* .node 解析为二进制插件模块
* 首次加载后的模块会缓存在require.cache中，所以多次使用require加载文件，得到的对象是同一个
